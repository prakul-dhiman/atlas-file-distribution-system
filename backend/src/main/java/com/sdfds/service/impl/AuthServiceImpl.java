...
public class AuthServiceImpl implements AuthService {
    ...
    private static final Set<String> COMMON_PASSWORDS = Set.of(
            "password123", "1234567890", "admin12345", "welcome123",
            "letmein123", "password1234", "qwerty12345", "iloveyou123"
    );

    private static final long VERIFICATION_TOKEN_TTL_SECONDS = 86400; // 24 hours
    private static final long PASSWORD_RESET_TOKEN_TTL_SECONDS = 3600; // 1 hour
    ...
    public AuthResponse register(RegisterRequest registerRequest) {
        log.info("Registration attempt for username: {}, email: {}", registerRequest.getUsername(), registerRequest.getEmail());
        ...
        auditLogService.log("REGISTER", "User registered: " + savedUser.getUsername(), null);
    ...
    public AuthResponse login(LoginRequest loginRequest) {
        ...
        auditLogService.log("LOGIN", "User logged in: " + user.getUsername(), user);
    ...
    public AuthResponse refreshToken(RefreshTokenRequest refreshTokenRequest) {
        ...
        if (token.getIsRevoked()) {
            log.warn("Revoked refresh token presented for user ID: {}", token.getUser().getId());
    ...
    public void requestPasswordReset(PasswordResetRequest request) {
        ...
        if (user != null) {
            ...
            PasswordResetToken resetToken = PasswordResetToken.builder()
                    ...
                    .expiryDate(Instant.now().plusSeconds(PASSWORD_RESET_TOKEN_TTL_SECONDS))
                    .isUsed(false)
                    .build();
            passwordResetTokenRepository.save(resetToken);
            emailService.sendPasswordResetEmail(user.getEmail(), user.getUsername(), resetToken.getToken());
            auditLogService.log("PASSWORD_RESET_REQUESTED", "Password reset requested for user: " + user.getUsername(), user);
    ...
    public void confirmPasswordReset(PasswordResetConfirmRequest request) {
        ...
        auditLogService.log("PASSWORD_RESET_CONFIRMED", "Password reset confirmed for user: " + user.getUsername(), user);
    ...
    public void logout(String refreshTokenStr) {
        if (refreshTokenStr != null) {
            refreshTokenRepository.findByToken(refreshTokenStr).ifPresent(token -> {
                token.setIsRevoked(true);
                refreshTokenRepository.save(token);
                auditLogService.log("LOGOUT", "User logged out: " + token.getUser().getUsername(), token.getUser());
                log.info("Refresh token revoked during logout for user ID: {}", token.getUser().getId());
            });
        }
    }
    ...
    public void verifyEmail(String token) {
        ...
        if (Boolean.TRUE.equals(verificationToken.getIsUsed()) || verificationToken.getExpiryDate().isBefore(Instant.now())) {
        ...
        auditLogService.log("EMAIL_VERIFIED", "User email verified: " + user.getEmail(), user);
    ...
    public void resendVerificationEmail(String email) {
        ...
        if (user == null || Boolean.TRUE.equals(user.getIsEmailVerified())) {
            log.info("Verification email resend skipped for non-existent or already verified email");
            return;
        }
        ...
        EmailVerificationToken verificationToken = EmailVerificationToken.builder()
                .user(user)
                .token(UUID.randomUUID().toString())
                .expiryDate(Instant.now().plusSeconds(VERIFICATION_TOKEN_TTL_SECONDS))
                .isUsed(false)
                .build();
        emailVerificationTokenRepository.save(verificationToken);
        emailService.sendVerificationEmail(user.getEmail(), user.getUsername(), verificationToken.getToken());
        auditLogService.log("VERIFICATION_RESENT", "Verification email resent for: " + user.getEmail(), user);
