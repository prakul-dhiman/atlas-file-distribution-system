...
class FileServiceTest {
    ...
    void uploadFile_Success() throws Exception {
        ...
        FileEntity initialEntity = FileEntity.builder()
                .id(100L)
                .name("test.txt")
                .sizeBytes(11L)
                .user(testUser)
        ...
        when(localStorageService.saveFile(
                eq(1L),
                eq(100L),
                any(InputStream.class)
        )).thenReturn(
                new LocalStorageService.WriteResult(
                        11L,
                        "dummy_checksum_sha256"
                )
        );
        ...
        verify(localStorageService).saveFile(
                eq(1L),
                eq(100L),
                any(InputStream.class)
        );
    ...
    void uploadFile_QuotaExceeded() {
        testUser.setUsedStorageBytes(5368709100L);
        ...
        assertTrue(
                ex.getMessage().contains("Storage quota exceeded")
        );
    }
    ...
    void deleteFile_Success() {
        ...
        when(fileRepository.findByIdAndUserAndIsTrashedFalse(
                100L,
                testUser
